...
class QuizStore {
    ...
    async nextQuestion(quizId, interaction) {
        ...
        if (quiz.currQuestion >= quiz.questions.length) {
            return; 
        }   
    ...
    async addPlayerAnswer(interaction) {
        ...
        } else if (this.quizzes[quizId].questions[questionId].isMultChoice) {
            // if they've already selected this answer, remove it...
            if (this.quizzes[quizId].questions[questionId].playerAnswers.answerIds.includes(answerId)) {
                this.quizzes[quizId].questions[questionId].playerAnswers.answerIds.remove(answerId);
            } else {
                // else add it 
                this.quizzes[quizId].questions[questionId].playerAnswers.answerIds.push(answerId);
            }  
        }
    ...
    async displayLeaderboard(quizId, interaction) {
        ...
        // display leadboard 
        let leaderboard = "";
        for (const playerScore of this.quizzes[quizId].playerScores) {
