...
class CarrotboardStorage {
    ...
    async generateLeaderboard({onlyFirstPage=false, userID=null, channelID, server_id}) {
        ...
        let emojiQuery = `select MIN_CARROT_COUNT from CB_SERVERS
            where SERVER_ID = $1`;
        ...
        if (userID != null) {
            entries = await this.db.get_all_by_user( minCarrotCount, userID);
        } else {
            entries = await this.db.get_all(minCarrotCount);
        }
