...
class CarrotboardStorage {
    ...
    async sendCBAlert(messageReaction, CBID, emoji, server_id) {
        ...
        alertChannelId = alertChannelId.rows[0].alert_channel_id;
        if (!alertChannelId) {
    ...
    async updateLeaderboard(server_id, channel_id) {
        // generate the leaderboard
        const embedPages = await this.generateLeaderboard({ onlyFirstPage: true, server_id: server_id, channelID: channel_id });
        ...
        if (permaChannelID?.rows.length == 0) {
            console.log('Couldnt find leaderboard id');
            return null;
        }
        permaChannelID = permaChannelID.rows[0].leaderboard_channel_id;
        console.log('leaderboard msg id: ', permaChannelID);
        if (!permaChannelID) {
            console.log('ERROR: unable to fetch leaderboard msg id!!');
            return;
        } else {
        ...
        let leaderboardMsgId = await this.db.executeDBQuery(messageChannelQuery, msgQueryValues);
        leaderboardMsgId = leaderboardMsgId.rows[0].leaderboard_msg_id;

        try {
            message = await channel.messages.fetch(leaderboardMsgId);
        } catch(e) {
            console.log('ERROR: unable to fetch leaderboard message!!');
            console.log(e);
            return;
        }
    ...
    async generateLeaderboard({onlyFirstPage=false, userID=null, channelID, server_id, debug}) {
