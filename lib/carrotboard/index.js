...
class CarrotboardStorage {
    ...
    async updateLeaderboard() {
        ...
        let channel;
        try {
            channel = await this._client.channels.fetch(this.db.getPermaChannelID);
        } catch {
            return;
        }
        ...
        let message;
        try {
            message = await channel.messages.fetch(this.db.getLeaderboardID);
        } catch {
            return;
        }
    ...
    async generateLeaderboard({onlyFirstPage=false, userID=null, channelID}) {
        // get the entries
        let entries = [];
        if (userID != null) {
            entries = await this.db.get_all_by_user( this.db.getMinCarrotCount(), userID);
        } else {
            entries = await this.db.get_all(this.db.getMinCarrotCount());
        }
        ...
        for (const entry of entries) {
            ...
            // get the url
            const guildID = this.db.getGuildID();
