...
class DBcarrotboard {
    ...
    // check if a server has an entry yet 
    async check_server(server_id) {
        const client = await this.pool.connect()
        try{
            //console.log("Running check_table command")
            await client.query("BEGIN");
            const values = [server_id]
            var result = await client.query("select * from CB_SERVERS where server_id=$1",values)
            await client.query("COMMIT");
    ...
    async count_values(message_id, user_id, channel_id) {
        const client = await this.pool.connect()
        try{
        ...
        console.log('counting values query defined');
        var query = `SELECT COUNT(*) FROM CB_SERVERS 
            JOIN CB_MESSAGES ON CB_SERVERS.SERVER_ID=CB_MESSAGES.SERVER_ID
            WHERE message_id = $1 and CB_MESSAGES.channel_id = $2`;

        var values = [ message_id, channel_id]
        var result = await client.query(query, values)
        await client.query("COMMIT");
        console.log('counting values query finished');
    ...
    async get_count(emoji,message_id, user_id, channel_id) {
        ...
        try{
            ...
            var query = `SELECT * FROM CB_SERVERS
            JOIN CB_MESSAGES ON CB_SERVERS.SERVER_ID = CB_MESSAGE.SERVER_ID
            where message_id = $1 and CB_SERVERS.channel_id = $2`;
    ...
    async add_value(emoji, server_id, message_id, user_id, channel_id,message_contents) {
        ...
        try{
            ...
            var count_val = await this.count_values(message_id, user_id, channel_id);
            ...
            if(count_val == 0) {
                ...
                console.log("Hiiiii I'm adding to my msg\n\n\n");
    ...
    async get_by_msg_id(message_id) {
        const client = await this.pool.connect()
        try{
            // console.log("Connected successfully.")
            await client.query("BEGIN");
